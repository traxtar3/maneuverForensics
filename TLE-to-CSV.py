...
def convertTLE(filename):
    ...
    with open(filename, mode='r') as infile:
        for line in infile:
            ...
            sat_dic.update({N: [jsec, position[0], position[1], position[2], velocity[0], velocity[1], velocity[2], line1.split(), line2.split()]})
...
def forensics(sat_dic):
    ...
    with open('large.csv', 'w') as f1:
        ...
        for i in arange(begin, final, 1):
            ...
            data = [back3, back2, back1, middle, fwd1, fwd2, fwd3]
            jetpack = sat_dic[i][7] + sat_dic[i][8] + data
            # print (str(jetpack[23]))
            # jetpack = int(jetpack[0]), str(jetpack[1]), str(jetpack[2]), float(jetpack[3]), float(jetpack[4]), str(jetpack[5]), str(jetpack[6]), float(jetpack[7]), float(jetpack[8]), float(jetpack[9]), float(jetpack[10]), float(jetpack[11]), float(jetpack[12]), float(jetpack[13]), float(jetpack[14]), float(jetpack[15]), float(jetpack[16]), float(jetpack[17]), float(jetpack[18]), float(jetpack[19]), float(jetpack[20]), float(jetpack[21]), float(jetpack[22]), float(jetpack[23]),

            # writer.writerow([''] + range(1000000))
            # for i in range(1000000):
            writer.writerow(jetpack)
            # print(jetpack)
