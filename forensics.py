...
def forensics(sat_dic, scc):
    ...
    df = pd.DataFrame(columns=['scc', 'time', 'middle'])
    ...
    for i in arange(begin, final, 1):
        ...
        middle = (back1 + fwd1) / 2
        df = df.append({'scc': scc, 'time': ref2, 'middle': middle}, ignore_index=True)
...
if __name__ == '__main__':
    ...
    df = final_data
    df.plot.line(x='time', y=toplot)
    # plt1 = sns.scatterplot(x='time', y=toplot, data=df, label=scc, marker='o')
